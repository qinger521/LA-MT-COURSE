...
@register_model('branch_transformer')
class BranchTransformerModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        encoder = BranchTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = BranchTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return BranchTransformerModel(encoder, decoder)




class BranchTransformerEncoder(FairseqEncoder):
    """
...
class BranchTransformerDecoder(FairseqIncrementalDecoder):
    """
...
class TransformerMBEncoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn_branches = nn.ModuleList([
            MultiheadAttention(
                ...
                dropout=args.attention_dropout,) if args.max_relative_length == -1
            else RelativeMultiheadAttention(
                self.embed_dim, args.encoder_attention_heads,
    ...
    def fc1fc2(self, x):
        ...
        for fc1, fc2 in zip(self.fc1_branches, self.fc2_branches):
            o = F.relu(fc1(x))
            o = F.dropout(o, p=self.relu_dropout, training=self.training)
            o = fc2(o)
            outputs.append(o)
...
class TransformerMBDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            self_attn_value = torch.mean(torch.stack([saved_state['prev_value'] for saved_state in saved_states]),
                                         dim=0)
...
def merge_branches(branch_outputs, branch_dropout, training):
    ...
    for branch_output_list_i in branch_output_lists:
        if branch_output_list_i[0] is None:
            ...
            branch_selection_d_expanded = branch_selection_d[
                (slice(None),) + tuple(None for _ in range(branch_output_i.ndimension() - 1))]
...
@register_model_architecture('branch_transformer', 'branch_transformer')
...
@register_model_architecture('branch_transformer', 'branch_transformer_iwslt_de_en')
...
@register_model_architecture('branch_transformer', 'branch_transformer_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('branch_transformer', 'branch_transformer_t2t_wmt_en_de')
...
@register_model_architecture('branch_transformer', 'branch_relative_transformer_wmt_en_de')
...
@register_model_architecture('branch_transformer', 'branch_relative_transformer_t2t_wmt_en_de')
...
@register_model_architecture('branch_transformer', 'branch_transformer_vaswani_wmt_en_de_big')
...
@register_model_architecture('branch_transformer', 'branch_transformer_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('branch_transformer', 'branch_transformer_wmt_en_de_big')
...
@register_model_architecture('branch_transformer', 'branch_transformer_wmt_en_de_big_t2t')
