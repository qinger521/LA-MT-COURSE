...
@register_model('generative_transformer')
class GenerativeTransformerModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        return GenerativeTransformerModel(encoder, decoder)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        if self.calculate_num > 1 and args.encoder_layers > 0:
            self.layer_trans = nn.ModuleList([])
            self.layer_trans.extend([
                EncoderLayerTrans(args)
                for i in range(args.encoder_layers)
            ])
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for index, layer in enumerate(self.layers):
            ...
            if self.calculate_num > 1:
                pre_layer = layer
                for _ in range(self.calculate_num - 1):
                    generative_layer = self.layer_trans[index].forward(pre_layer)
                    x = generative_layer.forward(x, encoder_padding_mask)
                    pre_layer = generative_layer
...
class GenerativeEncoderLayer():
    """Generative Encoder layer block.
    """

    def __init__(self, pre_layer, rpr=-1):
        super().__init__()
        self.dropout = pre_layer.dropout
        self.relu_dropout = pre_layer.relu_dropout
        self.normalize_before = pre_layer.normalize_before
        self.training = pre_layer.training

        if rpr == -1:
            self.self_attn = GenerativeMultiheadAttention(pre_layer.self_attn)
        else:
            self.self_attn = GenerativeRelativeMultiheadAttention(pre_layer.self_attn)
        self.fc1 = GenerativeLinear(pre_layer.fc1)
        self.fc2 = GenerativeLinear(pre_layer.fc2)
        self.layer_norms = [GenerativeLayerNorm(ln) for ln in pre_layer.layer_norms]
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        residual = x
        x = self.maybe_layer_norm(0, x, before=True)
        x, _ = self.self_attn.forward(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(0, x, after=True)
    ...
    def maybe_layer_norm(self, i, x, before=False, after=False):
        assert before ^ after
        if after ^ self.normalize_before:
            return self.layer_norms[i].forward(x)
        else:
            return x


class EncoderLayerTrans(nn.Module):
    """Encoder layer trans.
    """
...
class GenerativeMultiheadAttention():
    """Multi-headed attention.

    See "Attention Is All You Need" for more details.
    """

    def __init__(self, pre_layer_attn):
        super().__init__()
        self.training = pre_layer_attn.training

        self.embed_dim = pre_layer_attn.embed_dim
        self.num_heads = pre_layer_attn.num_heads
        self.dropout = pre_layer_attn.dropout
        self.head_dim = pre_layer_attn.head_dim
        self.scaling = pre_layer_attn.scaling

        self.in_proj_weight = pre_layer_attn.in_proj_weight
        self.in_proj_bias = pre_layer_attn.in_proj_bias
        self.out_proj = GenerativeLinear(pre_layer_attn.out_proj)

        self.bias_k = pre_layer_attn.bias_k
        self.bias_v = pre_layer_attn.bias_v

        self.add_zero_attn = pre_layer_attn.add_zero_attn

        self.onnx_trace = pre_layer_attn.onnx_trace
...
class GenerativeRelativeMultiheadAttention():
    ...
    def __init__(self, pre_layer_attn):
        ...
        self.embed_dim = pre_layer_attn.embed_dim
        self.num_heads = pre_layer_attn.num_heads
        self.max_relative_length = pre_layer_attn.max_relative_length
        self.dropout = pre_layer_attn.dropout
        self.head_dim = pre_layer_attn.head_dim
        self.k_only = pre_layer_attn.k_only
        self.scaling = pre_layer_attn.scaling
        ...
        self.relative_position_keys = pre_layer_attn.relative_position_keys
        if not self.k_only:
            self.relative_position_values = pre_layer_attn.relative_position_values
    ...
    def _relative_attention_inner(self, x, y, z, transpose=True):
        ...
        return attn
class GenerativeLinear():
    __constants__ = ['bias', 'in_features', 'out_features']

    def __init__(self, pre_layer_linear):
        super().__init__()
        self.in_features = pre_layer_linear.in_features
        self.out_features = pre_layer_linear.out_features
        self.weight = pre_layer_linear.weight
        self.bias = pre_layer_linear.bias
...
class GenerativeLayerNorm():

    __constants__ = ['normalized_shape', 'weight', 'bias', 'eps', 'elementwise_affine']

    def __init__(self, pre_layer_ln):
        super().__init__()
        self.normalized_shape = pre_layer_ln.normalized_shape
        self.eps = pre_layer_ln.eps
        self.elementwise_affine = pre_layer_ln.elementwise_affine
        self.weight = pre_layer_ln.weight
        self.bias = pre_layer_ln.bias
...
@register_model_architecture('generative_transformer', 'generative_transformer')
...
@register_model_architecture('generative_transformer', 'generative_transformer_test')
...
@register_model_architecture('generative_transformer', 'generative_transformer_t2t_iwslt_de_en')
...
@register_model_architecture('generative_transformer', 'generative_relative_transformer_t2t_iwslt_de_en')
def relative_transformer_t2t_iwslt_de_en(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.enc_calculate_num = 4
    args.max_relative_length = 8
    args.k_only = True
    transformer_t2t_iwslt_de_en(args)


@register_model_architecture('generative_transformer', 'generative_transformer_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('generative_transformer', 'generative_transformer_t2t_wmt_en_de')
...
@register_model_architecture('generative_transformer', 'generative_relative_transformer_wmt_en_de')
...
@register_model_architecture('generative_transformer', 'generative_relative_transformer_t2t_wmt_en_de')
def relative_transformer_t2t_wmt_en_de(args):
    ...
    args.encoder_layers = 6
    args.max_relative_length = 8
    args.enc_calculate_num = 8
    args.k_only = True
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani, et al, 2017)
@register_model_architecture('generative_transformer', 'generative_transformer_vaswani_wmt_en_de_big')
...
@register_model_architecture('generative_transformer', 'generative_transformer_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('generative_transformer', 'generative_transformer_wmt_en_de_big')
...
@register_model_architecture('generative_transformer', 'generative_transformer_wmt_en_de_big_t2t')
