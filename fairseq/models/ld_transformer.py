...
@register_model('ld_transformer')
class LayerDropTransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--encoder-layer-dropout', type=int, metavar='N',
    ...
    def build_model(cls, args, task):
        ...
        return LayerDropTransformerModel(encoder, decoder)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.encoder_layer_dropout = args.encoder_layer_dropout
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for layer in self.layers:
            ...
            if not self.training or (dropout_probability > self.encoder_layer_dropout):
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        if self.normalize:
            self.layer_norm = LayerNorm(embed_dim)
        self.decoder_layer_dropout = args.decoder_layer_dropout
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        for layer in self.layers:
            dropout_probability = random.uniform(0, 1)
            if not self.training or (dropout_probability > self.decoder_layer_dropout):
                x, attn = layer(
                    x,
...
@register_model_architecture('ld_transformer', 'ld_transformer')
...
@register_model_architecture('ld_transformer', 'ld_transformer_wmt_en_de')
def ld_transformer_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('ld_transformer', 'ld_transformer_t2t_wmt_en_de')
def ld_transformer_t2t_wmt_en_de(args):
...
@register_model_architecture('ld_transformer', 'ld_relative_transformer_wmt_en_de')
def ld_relative_transformer_wmt_en_de(args):
    args.max_relative_length = 8
    args.k_only = True
    base_architecture(args)


@register_model_architecture('ld_transformer', 'ld_relative_transformer_t2t_wmt_en_de')
def ld_relative_transformer_t2t_wmt_en_de(args):
    args.encoder_normalize_before = True
    args.decoder_normalize_before = True
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
